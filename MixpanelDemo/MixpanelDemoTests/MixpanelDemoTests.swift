...
class MixpanelDemoTests: MixpanelBaseTests {
    ...
    func testFlushQueueContainsCorruptedEvent() {
        stubTrack()
        MixpanelPersistence.sharedInstance.saveEntity(["event": "bad event1", "properties": ["BadProp": Double.nan]], type: .events, token: mixpanel.apiToken)
        MixpanelPersistence.sharedInstance.saveEntity(["event": "bad event2", "properties": ["BadProp": Float.nan]], type: .events, token: mixpanel.apiToken)
        MixpanelPersistence.sharedInstance.saveEntity(["event": "bad event3", "properties": ["BadProp": Double.infinity]], type: .events, token: mixpanel.apiToken)
        MixpanelPersistence.sharedInstance.saveEntity(["event": "bad event4", "properties": ["BadProp": Float.infinity]], type: .events, token: mixpanel.apiToken)
...
//            waitForTrackingQueue()
//            let peopleQueue_value = peopleQueue(token: mixpanel.apiToken)
//            let unidentifiedQueue = unIdentifiedPeopleQueue(token: mixpanel.apiToken)
...
//            waitForTrackingQueue()
//            let new_UnidentifiedQueue = unIdentifiedPeopleQueue(token: mixpanel.apiToken)
//            let new_peopleQueue = peopleQueue(token: mixpanel.apiToken)
...
//            XCTAssertTrue(new_UnidentifiedQueue.isEmpty,
//                          "identify: should move records from unidentified queue")
//            XCTAssertTrue(new_peopleQueue.count == 1,
//                          "identify: should move records to main people queue")
//            XCTAssertEqual(new_peopleQueue.last?["$token"] as? String,
//                           kTestToken, "incorrect project token in people record")
//            XCTAssertEqual(new_peopleQueue.last?["$distinct_id"] as? String,
//                           distinctId, "distinct id not set properly on unidentified people record")
//            let p: InternalProperties = new_peopleQueue.last?["$set"] as! InternalProperties
//            XCTAssertEqual(p["p1"] as? String, "a", "custom people property not queued")
//            assertDefaultPeopleProperties(p)
//            mixpanel.people.set(property: "p1", to: "a")
//            waitForTrackingQueue()
//            XCTAssertTrue(unIdentifiedPeopleQueue(token: mixpanel.apiToken).isEmpty,
//                          "once idenitfy: is called, unidentified queue should be skipped")
//            XCTAssertTrue(peopleQueue(token: mixpanel.apiToken).count == 2,
