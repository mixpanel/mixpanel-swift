    ...
    open var loggingEnabled: Bool = false {
        didSet {
            if loggingEnabled {
                ...
                MPLogger.info(message: "Logging Enabled")
            } else {
                MPLogger.info(message: "Logging Disabled")
    ...
    private init(
        ...
        if let reachability = MixpanelInstance.reachability {
            ...
            func reachabilityCallback(reachability: SCNetworkReachability,
                ...
                MPLogger.info(message: "reachability changed, wifi=\(wifi)")
            }
    ...
    deinit {
        ...
        if let reachability = MixpanelInstance.reachability {
            if !SCNetworkReachabilitySetCallback(reachability, nil, nil) {
                MPLogger.error(message: "\(self) error unsetting reachability callback")
            }
            if !SCNetworkReachabilitySetDispatchQueue(reachability, nil) {
                MPLogger.error(message: "\(self) error unsetting reachability dispatch queue")
            }
        }
#endif
    }
...
extension MixpanelInstance {
    ...
    public func identify(distinctId: String, usePeople: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if distinctId.isEmpty {
            MPLogger.error(message: "\(self) cannot identify blank distinct id")
            if let completion = completion {
                DispatchQueue.main.async(execute: completion)
            }
            return
    ...
    public func createAlias(_ alias: String, distinctId: String, usePeople: Bool = true, andIdentify: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if alias.isEmpty {
            MPLogger.error(message: "\(self) create alias called with empty alias")
            if let completion = completion {
                DispatchQueue.main.async(execute: completion)
            }
            return
        ...
        if alias != distinctId {
            ...
            MPLogger.error(message: "alias: \(alias) matches distinctId: \(distinctId) - skipping api call.")
            if let completion = completion {
                DispatchQueue.main.async(execute: completion)
            }
        }
...
extension MixpanelInstance {
    ...
    public func getGroup(groupKey: String, groupID: MixpanelType) -> Group {
        ...
        if !(group.groupKey == groupKey && group.groupID.equals(rhs: groupID)) {
            ...
            MPLogger.info(message: "groups dictionary key collision: \(key)")
